// Originally from https://github.com/grafana/grafana/blob/c58542348d/pkg/tsdb/prometheus/querydata/response.go
...
func (s *QueryData) parseResponse(ctx context.Context, q *Query, res *http.Response) (*backend.DataResponse, error) {
	defer func() {
...
func addMetadataToWideFrame(q *Query, frame *data.Frame) {
	if frame.Meta == nil {
		frame.Meta = &data.FrameMeta{}
	}
	frame.Meta.ExecutedQueryString = executedQueryString(q)
...
func processExemplars(q *Query, dr *backend.DataResponse) *backend.DataResponse {
...
func isExemplarFrame(frame *data.Frame) bool {
	rt := ResultTypeFromFrame(frame)
	return rt == ResultTypeExemplar
}
